    ...
    using GeoConvert.Matrix;
        ...
        public class GeoConvertor
        {
            ...
            public double[] LLAfromECEF(double x, double y, double z)
            {
                ...
                double eb = Math.Sqrt((WGS84a * WGS84a - WGS84b * WGS84b) / (WGS84b * WGS84b)); // Second eccentricity

                double p = Math.Sqrt(x * x + y * y);
                double theta = Math.Atan2(z * WGS84a, p * WGS84b);
                double lon = Math.Atan2(y, x);

                // Bowring's formula for initial parametric (reduced) latitude
                double beta = Math.Atan2(WGS84a * z, WGS84b * p);

                // Compute geodetic latitude using Bowring's formula
                double phi = Math.Atan2(
                    z + eb * eb * WGS84b * Math.Pow(Math.Sin(beta), 3),
                    p - ea * ea * WGS84a * Math.Pow(Math.Cos(beta), 3)
                );

                // Compute altitude above the ellipsoid
                double N = WGS84a / Math.Sqrt(1 - ea * ea * Math.Sin(phi) * Math.Sin(phi));
                double alt = p / Math.Cos(phi) - N;

                return new double[3] { phi * (180.0 / Math.PI), lon * (180.0 / Math.PI), alt };
            }
